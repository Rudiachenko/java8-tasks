...
class OrderStats {
     ...
     * Task 4 (⚫⚫⚫⚫⚪)
     * <p>
     * Given a stream of customers, return the map, where customer email is mapped to a number
     * of different credit cards he/she used by the customer.
     *
     * @param customers stream of customers
     * @return map, where for each customer email there is a long
     * referencing a number of different credit cards this customer uses.
     */
    ...
    static Optional<String> mostPopularCountry(final Stream<Customer> customers) {
        return customers
                .collect(Collectors.groupingBy(customer
                        -> customer.getAddress().getCountry(), Collectors.counting()))
                .entrySet().stream()
